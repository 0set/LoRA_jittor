...
# 创建一个 writer 对象
writer = SummaryWriter('runs/experiment_original_adam')

# influence model, calculate the influence score between two samples.
def print_args(args):
    # 注意：去除了对 args.rank 的引用
    print('=' * 100)
...
def optimizer_step(_loss, _optimizer, _model, _schedule, args, is_update=True):
    if args.fp16:
        with amp.scale_loss(_loss, _optimizer) as _scaled_loss:
            _optimizer.backward(_scaled_loss)
    else:
        _optimizer.backward(_loss)

    if is_update:
        if args.clip > 0:
            _optimizer.clip_grad_norm(args.clip)
    ...
    for idx, data in enumerate(train_loader):
        ...
        if train_step % args.log_interval == 0:
            ...
            log_str = f'| epoch {epoch:3d} step {train_step:>8d} | { idx + 1:>6d} batches | ' \
                      ...
                      f'ppl {math.exp(avg_lm_loss.avg):8.2f}'

            # 记录 lr, loss, avg loss 和 ppl
            writer.add_scalar('LR', lr, train_step)
            writer.add_scalar('Loss', avg_lm_loss.val, train_step)
            writer.add_scalar('Avg Loss', avg_lm_loss.avg, train_step)
            writer.add_scalar('PPL', math.exp(avg_lm_loss.avg), train_step)
